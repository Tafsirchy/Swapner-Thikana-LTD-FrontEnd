...
const AdminPropertiesPage = () => {
  ...
  const handleApprove = async (propertyId) => {
    // ✅ CRITICAL FIX: Add confirmation modal before approval
    if (
      !window.confirm(
        "Are you sure you want to APPROVE and PUBLISH this property? This action cannot be undone.",
      )
    ) {
      return;
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      // ✅ CRITICAL FIX: Wait for API response BEFORE updating UI
      ...
      // Refetch to ensure UI stays in sync with DB
      fetchProperties(currentPage);
    }
  };
  ...
  const handleToggleFeatured = async (propertyId, currentStatus) => {
    try {
      ...
      toast.error(`Featured update failed: ${errorMessage}`);
    }
  };
          ...
          <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-4">
            <div className="flex justify-between items-start gap-4">
              <div className="min-w-0 flex-1">
                <div className="font-bold text-zinc-100 line-clamp-2">
                  {property.title}
                </div>
              ...
              <span
                className={`${statusColors[property.status]} px-2 py-0.5 rounded-full font-bold text-[9px] uppercase tracking-wider`}
              >
                {property.status}
              </span>
            </div>
