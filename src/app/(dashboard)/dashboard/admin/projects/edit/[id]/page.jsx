...
const EditProjectPage = () => {
  ...
  const sessionGalleryIds = React.useRef(new Set());
  ...
  useEffect(() => {
    return () => {
      if (!isSubmitted && sessionGalleryIds.current.size > 0) {
        console.log('[EditProject] Cleaning up unsaved gallery items:', sessionGalleryIds.current.size);
        sessionGalleryIds.current.forEach(id => {
          api.uploads.delete(id).catch(err => console.error('Edit cleanup failed:', err));
        });
      }
    };
  }, [isSubmitted]);
